...
export const SideBar: React.FC<{ isClosed: boolean }> = ({ isClosed }) => {
  ...
  return (
    <>
      <div className={`h-screen px-4 py-4 gap-2 flex flex-col justify-between` + (isClosed ? " w-36 items-center" : " w-64")}>
        <div className="flex flex-col gap-2">
          <div>
            <header className="flex flex-col items-start justify-between">
              <img
                src="/src/assets/svg/logo_with_text.svg"
                alt="Logo"
                className="py-3"
              />
          ...
          <div className="pb-3">
            {!isClosed &&
            <span className="text-gray-500 text-sm py-1 px-3 h-7">
              Developer
            </span>
            }
            <SideBarItemsGroup
          ...
          <div>
            {!isClosed &&
            <span className="text-gray-500 text-sm py-1 px-3 h-7">Account</span>
      }
            <SideBarItemsGroup
              isClosed={isClosed}
              items={[
            ...
            />
            {
            <div className={`flex px-2 rounded-lg border-s-transparent transition-all ease-in-out cursor-pointer relative z-10 items-center text-gray-700 h-9${isClosed ? " justify-center" : ""}`}>
              <span className="m-1">
                <TablerIcon className="text-gray-900"/>
              </span>
              {!isClosed && (
                <span
        ...
        <div className="flex flex-col justify-between">
          <div className="h-[1px] bg-[#D2D6DB] w-full mb-3 mt-2"></div>
            <div className={`flex flex-row justify-between items-start text-white px-3 py-2 rounded-[8px] border border-[#414651] bg-[linear-gradient(45deg,_#101828_0%,_#535862_100%)] shadow-[0px_1px_2px_0px_#0A0D120D]`}>
              <div className={`flex gap-1 flex-col`}>
                <span className="text-gray-200 text-[10px] leading-[14px] tracking-[0]">
                  System Health Status:
                </span>
                <span className="font-bold text-[12px] leading-[18px] tracking-[0]">Normal</span>
              </div>
